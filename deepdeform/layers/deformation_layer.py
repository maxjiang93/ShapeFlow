...
class NeuralFlowModel(nn.Module):
    def __init__(self, dim=3, latent_size=1, f_nlayers=4, f_width=50,
                 s_nlayers=3, s_width=20, nonlinearity='relu', conformal=False,
                 arch='imnet', no_sign_net=False, divfree=False, symm_dim=None):
...
class NeuralFlowDeformer(nn.Module):
    def __init__(self, dim=3, latent_size=1, f_nlayers=4, f_width=50,
        ...
        self.net = NeuralFlowModel(dim=dim, latent_size=latent_size,
                                   ...
                                   divfree=divfree, symm_dim=symm_dim)
        if symm_dim is not None:
            if not (isinstance(symm_dim, int) or isinstance(symm_dim, list)):
                raise ValueError("symm_dim must be int or list of ints, indicating axes of symmetry.")
    ...
    def forward(self, points, latent_sequence):
        ...
        Returns:
          points_transformed: tensor of shape [batch, num_points, dim] if not self.return_waypoint.
                              tensor of shape [nsteps, batch, num_points, dim] if self.return_waypoint.
        """
